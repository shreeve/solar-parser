...
class Generator {
  ...
  processGrammar(grammar) {
    if (grammar.operators)
      this._processOperators(grammar.operators);
  ...
  _generateActionCode(actionGroups) {
    ...
    for (const action in actionGroups) {
      if (!Object.hasOwn(actionGroups, action))
        continue;
  ...
  _buildTokenMappings() {
    ...
    for (const name in this.symbolIds) {
      if (!Object.hasOwn(this.symbolIds, name))
        continue;
  ...
  _computeNullableSets() {
    ...
    while (changed) {
      ...
      for (const symbol in this.types) {
        if (!Object.hasOwn(this.types, symbol))
          continue;
    ...
    while (changed) {
      ...
      for (const rule of this.rules) {
        for (let i = 0; i < rule.symbols.length; i++) {
          ...
          if (this.types[symbol].follows.size > oldSize)
            changed = true;
        }
      }
    }
      ...
      for (const item of itemSet.reductions) {
        for (const stackSymbol of item.lookaheads) {
          ...
          if (action) {
            ...
            const solution = this._resolveConflict(item.rule, op, [REDUCE, item.rule.id], which);
...
`;
  ...
  parse(input) {
    ...
    while (true) {
      state = stk.at(-1);
      action = this.defaultActions[state] || (symbol == null && (symbol = lex()),
